        ...
        switch (type)
        {
            case CellType.String:
                if (fieldType.IsEnum)
                {
                    ...
                    /// 숫자 값이 들어오면 enum으로 변환
                    if (int.TryParse(str, out int intVal))
                        return Enum.ToObject(fieldType, intVal);

                    /// [Flags] enum 지원: "FlagA|FlagB"
                    if (str.Contains("|"))
                    {
                        int total = 0;
                        foreach (var part in str.Split('|'))
                        {
                            total |= (int)Enum.Parse(fieldType, part.Trim(), true);
                        }
                        return Enum.ToObject(fieldType, total);
                    }

                    /// 기본 문자열 파싱 (대소문자 무시)
                    return Enum.Parse(fieldType, str, true);
                }
            ...
            case CellType.Numeric:
                if (fieldType.IsEnum)
                    return Enum.ToObject(fieldType, (int)cell.NumericCellValue);
